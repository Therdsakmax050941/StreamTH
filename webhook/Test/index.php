...
function getProductsFromDatabase($product_name) {
    ...
    $sql = "SELECT * FROM products WHERE product_data = :product_name"; // แก้ SQL นี้เพื่อเลือกแถวที่มี product_data ตรงกับค่าที่ส่งมา

    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(":product_name", $product_name, PDO::PARAM_STR); // ผูกค่า $product_name เข้ากับพารามิเตอร์ :product_name
    ...
    foreach ($products as $product) {
        $column = array(
            ...
            "action" => array(
                "type" => "message",
                "label" => "เลือก",
                "text" => $product["actions"][0]["text"]//$product["product_name"] // ตรวจสอบว่า $product["product_name"] มีค่าถูกต้อง
            )
        );
        $columns[] = $column;
    }
